...
for($i=0;$i<$arrGugun['body']['totalCount'];$i++){
	if($arrGugun['body']['totalCount'] < 2){
		if( $arrGugun['body']['items']['item']['code']==$_POST['gogon'] ){
			$gugunSelected = "selected";
		}							
		echo '<option value="'.$arrGugun['body']['items']['item']['code'].'" '.$gugunSelected.'>'.$arrGugun['body']['items']['item']['name'].'</option>';
	}else{
		if( $arrGugun['body']['items']['item'][$i]['code']==$_POST['gogon'] ){
			$gugunSelected[$i] = "selected";
		}							
		echo '<option value="'.$arrGugun['body']['items']['item'][$i]['code'].'" '.$gugunSelected[$i].'>'.$arrGugun['body']['items']['item'][$i]['name'].'</option>';
	}
}
