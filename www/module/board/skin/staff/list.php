		...
		<dl>
			...
			<dd>
				...
				<select name="category" id="category">
					...
					<?php
						$arrDepart[$arrDepartList["list"][$i]["idx"]] = $arrDepartList["list"][$i]["subject"];
					}
					?>
				</select>
			</dd>
				...
				<table>
					...
					<thead>
						<tr>	
							...
							<th class="pc_vw">진료과</th>
							<th class="pc_vw">의료진명</th>
					...
					?>
						<tr>
							...
							<td><i class="mo_vw">이미지</i><img src="<?=$imgsrc[$i]?>" style="max-width:100%"></td>
							<td><i class="mo_vw">진료과</i><?=$arrDepart[$arrBoardList["list"][$i]['category']]?></td>
							<td><i class="mo_vw">의료진명</i><?=$arrBoardList["list"][$i]['subject']?></td>
							<td><i class="mo_vw">사용여부</i><?=$arrBoardList["list"][$i]['is_show']?></td>			
							<td class="mono_btm"><i class="mo_vw">관리</i>
								<div class="btns">
									<a href="<?=$_SERVER["PHP_SELF"]?>?boardid=staff_break&mode=list&category=<?=$arrBoardList["list"][$i]['idx']?>" class="btn">휴진</a>
