...
<style>
	...
	.inner_table > tbody > tr > th
	{background:#0348a5;color:white;width:auto;}
</style>
<div class="container">
	...
	<div class="inbox write_tbl mo_break_write">
		...
		<table>
			<tr>
				...
				<td>
					<select name="category" id="category">
						<?php
						for($i=0;$i<$arrDepartList["list"]["total"];$i++){
						?>
						<option value="<?=$arrDepartList["list"][$i]["idx"]?>" <?=$arrDepartList["list"][$i]["idx"] == $arrBoardArticle["list"][0]["category"]?"selected":""?>><?=$arrDepartList["list"][$i]["subject"]?></option>
						<?php
						}
						?>
					</select>
			...
			<tr style="display:none;">
				<th>날짜</th>
				<td><input type="text" class="w3 datepicker" name="schedule_date" value="<?=date("Y-m-d")?>" maxlength="10" /></td>
			</tr>
			<tr style="display:none;">
				<th>공지</th>
				<td><div class="inputs">
			...
			<tr>
				<th>영문 전문진료분야</th>
				<td>
					<textarea id="etc_9" name="etc_9" cols="100" rows="15"><?=stripslashes($arrBoardArticle["list"][0]['etc_9'])?></textarea>
					<?
					//$CKContent = "contents";
					//include $_SERVER['DOCUMENT_ROOT'] . "/ckeditor/Editor.php";
					?>
			...
			<?php
				...
				$arrEtc_2 = json_decode($arrBoardArticle["list"][0]['etc_2'],true);
			?>
			<tr>
				<th>진료시간표</th>
				<td>
					<table class="inner_table" style="width:100%;">
						...
						<tbody>
							<tr>
								...
								<td>
									<select name="etc_2[]" >
										<?php foreach($arrEtc_2option as $key => $val){?>
										<option value="<?=$key?>" <?=$arrEtc_2[$i] == $key?"selected":""?>><?=$val?></option>
										<?php } ?>
									</select>
								</td>
								<?php } ?>
					...
					<label class="check"><input type="checkbox" name="etc_3" value="Y" <?if($arrBoardArticle["list"][0]['etc_3'] == "Y"){echo " checked";}?>><i></i>장기휴진</label>
					<input type="text" class="w4" name="etc_4" maxlength="200" value="<?=stripslashes($arrBoardArticle["list"][0]['etc_4'])?>">
				</td>
			</tr>
			...
			<tr>
				<th>학력/경력/주요활동</th>
				<td>
					<textarea id="etc_5" name="etc_5" cols="100" rows="15"><?=stripslashes($arrBoardArticle["list"][0]['etc_5'])?></textarea>
					<?
					$CKContent = "etc_5";
					include $_SERVER['DOCUMENT_ROOT'] . "/ckeditor/Editor.php";
					?>
			...
			<tr>
				<th>연구경력</th>
				<td>
					<textarea id="etc_6" name="etc_6" cols="100" rows="15"><?=stripslashes($arrBoardArticle["list"][0]['etc_6'])?></textarea>
					<?
					$CKContent = "etc_6";
					include $_SERVER['DOCUMENT_ROOT'] . "/ckeditor/Editor.php";
					?>
			...
			<tr>
				<th>저서</th>
				<td>
					<textarea id="etc_7" name="etc_7" cols="100" rows="15"><?=stripslashes($arrBoardArticle["list"][0]['etc_7'])?></textarea>
					<?
					$CKContent = "etc_7";
					include $_SERVER['DOCUMENT_ROOT'] . "/ckeditor/Editor.php";
					?>
			...
			<tr>
				<th>영상</th>
				<td>
					<textarea id="etc_8" name="etc_8" cols="100" rows="15"><?=stripslashes($arrBoardArticle["list"][0]['etc_8'])?></textarea>
					<?
					$CKContent = "etc_8";
					include $_SERVER['DOCUMENT_ROOT'] . "/ckeditor/Editor.php";
					?>
