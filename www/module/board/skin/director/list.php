				...
				<table>
					...
					?>
						<tr>
							...
							<td><i class="mo_vw">분류</i><?=$arrCategory[$arrBoardList["list"][$i]['category']]?></td>
							<td><i class="mo_vw">이름</i><?=$arrBoardList["list"][$i]['subject']?></td>
...
if($arrBoardList["list"]["total"] > 0){
	for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
		$arrDirector[$arrBoardList["list"][$i]["category"]][] = $arrBoardList["list"][$i];
	}
}
?>
	<div class="tit-box">
		<h3 class="tit-bullet heading1">역대병원장</h3>
		<p class="sub">지금의 고신대복음병원을 만들어주신 소중한 인연입니다.</p>
	</div>
	<?php
		$type = true;
		foreach($arrCategory as $key => $val){
	?>
	<div class="section btm-padding <?=$type?"":"bg-light "?>">
		<p class="stitle2"><?=$val?></p>
		<div class="history-prc">
			<?php 
			for($i=0;$i<count($arrDirector[$key]);$i++){
				
				$imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrDirector[$key][$i]['re_name'];
				if(!$arrDirector[$key][$i]['re_name']){$imgsrc[$i] = "/pub/images/sub/sample_pricipal.jpg";}
			?>
			<div class="rad-15">
				<div class="img"><img src="<?=$imgsrc[$i]?>" alt=""></div>
				<div class="text">
					<p class="stitle3"><?=$arrDirector[$key][$i]['subject']?></p>
					<p class="period"><?=$arrDirector[$key][$i]['contents']?></p>
				</div>
				<div class="flag secondary"><?=$arrDirector[$key][$i]['etc_1']?></div>
			</div>
			<?php } ?>
		</div>
	</div>
