...
//				document.getElementById(pr_Add1).value = extraAddr;
            ...
            <form name="form1" method="post" action="/module/board/board_evn.php" ENCTYPE="multipart/form-data">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex">
                                ...
                                <div class="btns" style="justify-content:flex-start; align-items :center; margin:0 !important; padding:0;"><button class="btn btn_save" type="button" onclick="OpenPersonView('equ')" style="margin:0;">장비검색</button></div>
                            </div>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="inputs">
                                <select name="usage_people" id="usage_people" style="width: 70px">
                                    <option value="">선택</option>
									<?php for ($i = 1; $i <= 50; $i++): ?>
                                        <option value="<?= $i ?>" <?= $arrBoardArticle["list"][0]['usage_people'] == $i ? 'selected' : '' ?>><?= $i ?>명</option>
									<?php endfor; ?>
                                </select>
                            </div>
                        </td>
                    ...
                    <tr>
                        <th>사용일수<span>*</span></th>
                        <td>
                            <div class="inputs">
                                <input type="text" name="usage_day" id="usage_day" value="<?=$arrBoardArticle["list"][0]['usage_day']?>" class="w4" readonly>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <th>대여일/반납일<span>*</span></th>
                        <td>
                            <div class="inputs">
                                <input type="text" name="rental_start_date" id="rental_start_date" class="w2 datepicker" value="<?= stripslashes($arrBoardArticle["list"][0]['rental_start_date']) ?>">
                                <em>&nbsp;~&nbsp;</em>
                                <input type="text" name="rental_end_date" id="rental_end_date" class="w2 datepicker" value="<?= stripslashes($arrBoardArticle["list"][0]['rental_end_date']) ?>">
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <th>대여시간/반납시간<span>*</span></th>
                        <td>
                            <div class="inputs">
                                <select name="rental_start_time" id="rental_start_time" class="w2">
									...
									<?php for ($hour = 9; $hour <= 22; $hour++): ?>
                                        ...
                                        <option value="<?= $time ?>" <?= $arrBoardArticle["list"][0]['rental_start_time'] == $hour ? 'selected' : '' ?>><?= $time ?></option>
									<?php endfor; ?>
                                </select>
                                <em>&nbsp;~&nbsp;</em>
                                <select name="rental_end_time" id="rental_end_time" class="w2">
									...
									<?php for ($hour = 9; $hour <= 22; $hour++): ?>
										<?php $time = sprintf('%02d:00', $hour); ?>
                                        <option value="<?= $time ?>" <?= $arrBoardArticle["list"][0]['rental_end_time'] == $hour ? 'selected' : '' ?>><?= $time ?></option>
									<?php endfor; ?>
                                </select>
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="inputs">
                                ...
                                <div id="fileAddWrap" style="display:none;width: 100%;">
                                    ...
                                    <?php if ($arrBoardArticle["total_files"] == 0): ?>
    ...
    <script language="javascript">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const usageDayInput = document.getElementById('usage_day');
            ...
            function calculateUsageDays() {
                ...
                if (startDate && endDate && !isNaN(startDate) && !isNaN(endDate)) {
                    ...
                    const dayDifference = Math.floor(timeDifference / (1000 * 3600 * 24)) + 1; // 시작일 포함
                    usageDayInput.value = dayDifference;
            ...
            function calculateTotalAmount() {
                ...
                const usageDays = parseFloat(usageDayInput.value) || 0;
                const totalPrice = usageDays * hourlyRate; // hourlyRate는 장비 일일 대여 요금

                document.getElementById('totalamount').value = totalPrice;
                calculateDiscount(); // 할인 재계산
            }
            ...
            function calculateDiscount() {
                ...
                const rentalDiscountSelect = document.getElementById('rental_discount');
                const discountValue = parseFloat(discountSelect.value) || 0;
                const rentalDiscountValue = parseFloat(rentalDiscountSelect.value) || 0;
                const totalAmount = parseFloat(document.getElementById('totalamount').value) || 0;
                ...
                if (rentalDiscountValue) {
                    const additionalDiscountAmount = finalAmount * (rentalDiscountValue / 100);
            ...
            // 기존 input change 이벤트도 유지 (직접 입력을 위해)
            startDateInput.addEventListener('change', calculateUsageDays);
            endDateInput.addEventListener('change', calculateUsageDays);
    ...
    <script type="text/javascript">
        ...
        {
            var requestUrl = "/backoffice/module/board/pop_board_view.php?boardid=equ&fname="+fname;	// 일반게시판
        ...
        function fnGoodSelect(stridx, inputName) {
            ...
            if (inputName == "equ") {
                $.post("/module/board/ajax_equ.php", { mid: stridx },
                    function(data) {
                        var frm = document.form1;
                        const equInfo = data.split("|");
                        frm.subject.value = equInfo[0]; // 장비명
                        frm.fee.value = equInfo[1];
                        frm.equ_idx.value = equInfo[2];
                        frm.category1.value = equInfo[3];
                        frm.category2.value = equInfo[4];
                        document.getElementById('finalAmountDisplay').innerText = '결제금액 : ' + parseFloat(equInfo[1]).toLocaleString() + '원';

                        // Update the global totalamount variable
                        totalamount = parseFloat(equInfo[1]);
                        finalamount = parseFloat(equInfo[1]);

                        // Assign equInfo[1] to hourlyRate
                        hourlyRate = parseFloat(equInfo[1]);
                    }
                );
            }
        }
