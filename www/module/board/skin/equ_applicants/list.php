...
if($_SESSION[$_SITE["DOMAIN"]]["ADMIN"]["ID"] && $_SERVER["PHP_SELF"]=="/backoffice/module/board/board_view.php"){
    ...
    //분류 리스트
	$arrCategory1 = getCategoryList(35);
	...
	<input type="hidden" name="category" value="<?=$_GET["category"]?>">
        ...
        <div class="inbox top_search">
            <dl class="search_wrap">
                ...
                <dd>
                <dd>
                    <select name="user_level" class="text" style="width:120px;">
                        <option value="">전체</option>
                        <?php foreach ($arrayLevel as $level_no => $level_name): ?>
                            <option value="<?= $level_no ?>" <?= $_GET['user_level'] == $level_no ? "selected" : "" ?>><?= $level_name ?></option>
                        <?php endforeach; ?>
                    </select>
                </dd>
                </dd>
...
<!--                <dt>대여일/반납일</dt>-->
<!--                <dd>-->
<!--                    <input type="text" class="datepicker" name="rental_start_date" value="--><?php //=$_GET['rental_start_date']?><!--" autocomplete="off"/><em>~</em>-->
<!--                    <input type="text" class="datepicker" name="rental_end_date" value="--><?php //=$_GET['rental_end_date']?><!--" autocomplete="off"/>-->
<!--                </dd>-->
<!--            </dl>-->
            ...
            <dl class="search_wrap">
                ...
                <dd>
                    <select name="sw" style="width:120px;">
                        ...
                        <option value='s'<?=$_GET['sw']=="s"?" selected='selected'":""?>>장비명</option>
                        <option value='n'<?=$_GET['sw']=="n"?" selected='selected'":""?>>이름</option>
                        <option value='email'<?=$_GET['sw']=="email"?" selected='selected'":""?>>이메일(아이디)</option>
	...
	<div class="inbox">
        <div class="tab_div" style="user-select: auto !important;">
            <div class="tab_menu cal" onclick="location.href='/backoffice/module/board/board_view.php?boardid=edu_applicants'" style="user-select: auto !important;">캘린더 보기</div>
        </div>
		<div class="tab_div">
			<div class='tab_menu <?=$_GET["status"] == ""?"on":""?>' onclick="location.href='<?=$_SERVER["PHP_SELF"]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>'">전체</div>
			<?php foreach($arrCategory as $key => $val){?>
			<div class='tab_menu <?=$_GET["status"] == $val?"on":""?>' onclick="location.href='<?=$_SERVER["PHP_SELF"]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&status=<?=$val?>'"><?=$val?></div>
			<?php } ?>
		</div>
		<div class="bdr_top">
			<div class="left">
			...
			<div class="bdr_right">
                <div class="btns">
                    <a href="#this" class="excel" download>엑셀파일로 저장<span class="pc_vw"></span></a>
                </div>
				<div class="count">
...
<!-- mo_break_tbl : 767px 이하에서 테이블 구조를 깰 때 사용합니다. -->
				...
				<table>
					...
					<thead>
						<tr>	
							...
							<th class="pc_vw">대여일/반납일</th>
							<th class="pc_vw">사용일수</th>
                            <th class="pc_vw">사용인원</th>
                            <th class="pc_vw">신청일</th>
                            <th class="pc_vw">대여형태</th>
                            <th class="pc_vw">신청상태</th>
                            <th class="pc_vw">비고</th>
                            <th class="pc_vw">출력</th>
					...
					?>
						<tr data-order="<?=$arrBoardList['list'][$i]['idx']?>">
							<td style="width:15%;"><label class="check notxt"><input type="checkbox" value="<?=$arrBoardList["list"][$i]['idx']?>" name="chk_list"><i></i></label></td>
                            <td style="width:5%;"><?=$arrBoardList["list"][$i]['app_no']?></td>
                            <td style="width:10%;"><a href="/backoffice/module/member/member_info.php?user_id=<?=$arrBoardList["list"][$i]['w_user']?>"><?=$arrBoardList["list"][$i]['name']?></a></td>
                            <td style="width:40%;"><a href="/backoffice/module/board/board_view.php?boardid=equ&mode=modify&idx=<?=$arrBoardList["list"][$i]['equ_idx']?>"><?=$arrBoardList["list"][$i]['subject']?></a></td>
                            <td style="width:10%;"><?=$user_level?></td>
							<td style="width:10%;"><?=$arrBoardList["list"][$i]['w_user']?></td>
							<td style="width:5%;"><?=$arrBoardList["list"][$i]['tel']?></td>
                            <td>
                            ...
                            <td style="width:5%;"><?=$arrBoardList["list"][$i]['rental_start_date']?> ~ <?=$arrBoardList["list"][$i]['rental_end_date']?></td>
							<td style="width:40%;"><?=$arrBoardList["list"][$i]['usage_day']?></td>
                            <td style="width:40%;"><?=$arrBoardList["list"][$i]['usage_people']?></td>
                            <td style="width:40%;"><?=date("Y-m-d",strtotime($arrBoardList["list"][$i]['wdate']))?></td>
							<td style="width:15%;"><?=$arrBoardList["list"][$i]['rental_type']?></td>
                            <td style="width:15%;"><?=$arrBoardList["list"][$i]['status']?></td>
                            <td style="width:15%;" title="<?=$arrBoardList["list"][$i]['contents']?>"><?= mb_strimwidth($arrBoardList["list"][$i]['contents'], 0, 20, '...') ?></td>
                            <td style="width:10%;">
                                <div class="btns">
                                    <a href="javascript:void(0);" onclick="fnPrint('<?=$arrBoardInfo["list"][0]["boardid"]?>','<?=$arrBoardList["list"][$i]['idx']?>')" class="btn perf">출력</a>
                                </div>
                            </td>
							<td style="width:10%;">
								<div class="btns">
		...
		<div class="bdr_btm">
            ...
            <div class="btns" style="right:unset;">
                <a href="javascript:void(0);" onclick="getSelections()" class="btn btn_del">문자발송</a>
                <select name="sw" style="width:120px;">
<!--                    <option value='all'--><?php //=$_GET['sw']=="all"?" selected='selected'":""?><!-->선택</option>-->
                </select>
                <a href="javascript:void(0);" onclick="getSelections()" class="btn btn_del">이메일발송</a>
                <select name="sw" style="width:120px;">
<!--                    <option value='all'--><?php //=$_GET['sw']=="all"?" selected='selected'":""?><!-->선택</option>-->
                </select>
            ...
            <div class="btns">
                ...
                <a href="<?=$_SERVER["PHP_SELF"]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=write&category=<?=$_GET['category']?>" class="btn">신청등록</a>
            </div>
		</div>
	</div>
</div>
<script type="text/javascript">
    // 출력
    function fnPrint(boardid, idx) {
        window.open('/module/board/print_equ.php?boardid=' + boardid + '&idx=' + idx);
    }
//<![CDATA[
$(document).ready(function(){
//달력
