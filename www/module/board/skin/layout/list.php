...
<script language="javascript">
	let arrEtc_1 = Array();
<?php
	foreach($arrEtc_1 as $key => $arrVal){
?>
	arrEtc_1["<?=$key?>"] = Array();
<?php
		foreach($arrVal as $subKey => $val){
?>
			arrEtc_1["<?=$key?>"]["<?=$subKey?>"] = "<?=$val?>";
<?php
		}
}	
?>

function changeCategory(val,selected = ""){
	$("#etc_1").html("<option value=''>전체</option>");
	for(key in arrEtc_1[val]){
		if(selected == key){
			$("#etc_1").append("<option value='"+key+"' selected>"+arrEtc_1[val][key]+"</option>");
		}else{
			$("#etc_1").append("<option value='"+key+"' >"+arrEtc_1[val][key]+"</option>");
		}
	}
...
$(document).ready(function(){
	changeCategory("<?=$_GET["category"] == ""?"":$_GET["category"]?>","<?=$_GET["etc_1"]?>");
})
</script>
		...
		<dl>
			...
			<dd>
				<select name="category" id="category" onchange="changeCategory(this.value)">
					...
					?>
					<option value="<?=$key?>" <?=$key == $_GET["category"]?"selected":""?>><?=$val?></option>
					<?php
					}
					?>
				</select> ->
				...
				<table>
					...
					<thead>
						<tr>	
							...
							<th class="pc_vw">동 - 층</th>
							<th class="pc_vw">시설명</th>
					...
					?>
						<tr>
							...
							<td><i class="mo_vw">동 - 층</i><?=$arrCategory[$arrBoardList["list"][$i]['category']]?> - <?=$arrEtc_1[$arrBoardList["list"][$i]['category']][$arrBoardList["list"][$i]['etc_1']]?></td>
							<td><i class="mo_vw">시설명</i><?=$arrBoardList["list"][$i]['subject']?></td>
