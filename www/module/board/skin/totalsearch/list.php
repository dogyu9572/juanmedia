...
for($i=0;$i<$arrBoardInfoList["total"];$i++){
	$arrEtc_3[$arrBoardInfoList["list"][$i]["boardid"]] = $arrBoardInfoList["list"][$i]["boardname"];
}

$arrCategoryInfoList = getArticleList("tbl_category_info", 0, 0, "");

for($i=0;$i<$arrCategoryInfoList["total"];$i++){
	$arrEtc_4[$arrCategoryInfoList["list"][$i]["categoryid"]] = $arrCategoryInfoList["list"][$i]["categoryname"];
}
?>
<div class="container">
			...
			<dd>
				<select name="category" id="category">
					...
					<?php for($i=0;$i<$arrMenuList["total"];$i++){?>
						<option value="<?=$arrMenuList["list"][$i]["cat_no"]?>" <?=$arrMenuList["list"][$i]["cat_no"] == $_GET["category"]?>><?=$arrMenuList["list"][$i]["cat_name"]?></option>
					<?php 
							$arrTopMenu[$arrMenuList["list"][$i]["cat_no"]] = $arrMenuList["list"][$i]["cat_name"];
						} 
					?>
				</select>
			</dd>
				...
				<table>
					...
					<thead>
						<tr>	
							...
							<th class="pc_vw">대메뉴</th>
							<th class="pc_vw">중/소메뉴</th>
							<th class="pc_vw">타입</th>
							<th class="pc_vw">제목/게시판/코드</th>
					...
					if($arrBoardList["list"]["total"] > 0){
						for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
							...
							$subject = $arrBoardList["list"][$i]['subject'];
							if($arrBoardList["list"][$i]['etc_2'] == "1"){ // html
								$subject = $arrBoardList["list"][$i]['subject'];
							}else if($arrBoardList["list"][$i]['etc_2'] == "2"){ // 게시판
								$subject = $arrEtc_3[$arrBoardList["list"][$i]['etc_3']];
							}else if($arrBoardList["list"][$i]['etc_2'] == "3"){ // 카테고리
								$subject = $arrEtc_4[$arrBoardList["list"][$i]['etc_4']];
							}
					?>
						<tr>
							...
							<td><i class="mo_vw">대메뉴</i><?=$arrTopMenu[$arrBoardList["list"][$i]['category']]?></td>
							<td><i class="mo_vw">중/소메뉴</i><?=$arrSubMenu[$arrBoardList["list"][$i]['category']][$arrBoardList["list"][$i]['etc_1']]?></td>
							<td><i class="mo_vw">타입</i><?=$arrEtc_2[$arrBoardList["list"][$i]['etc_2']]?></td>
							<td><i class="mo_vw">제목</i><?=$subject?></td>
