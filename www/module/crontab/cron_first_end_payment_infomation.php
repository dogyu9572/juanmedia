...
$dblink = SetConn($_conf_db["main_db"]);
	...
	// 1순위 최초마감일이거나 2순위 최초마감일 마지막 시일인 경우
	$Query = "select * from tbl_board_evaluation where (pay1_f_edate = '".$now_date."' OR pay2_f_edate = '".$now_date."')";
	...
	if($arrEvaluationList["total"] > 0){
		for($i=0;$i<$arrEvaluationList["total"];$i++){
			if($arrEvaluationList["list"][$i]["pay1_f_edate"] == $now_date){ // 1순위 발표일이라면
			...
			}else if($arrEvaluationList["list"][$i]["pay2_f_edate"] == $now_date){ // 2순위 발표일이라면
