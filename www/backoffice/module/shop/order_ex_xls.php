...
$filename = iconv("UTF-8","EUC-KR",$_SITE['NAME'])."_".iconv("UTF-8","EUC-KR","양식")."_".date(m).date(d).date(h).date(i).".xls";
header( "Content-type: application/vnd.ms-excel" ); 
header( "Content-type: application/vnd.ms-excel; charset=utf-8");
header( "Content-Disposition: attachment; filename =".iconv("UTF-8","EUC-KR",$_SITE['NAME'])."_".iconv("UTF-8","EUC-KR","양식")."_".date(m).date(d).date(h).date(i).".xls" ); 
...
for ( $i=0 ; $i < $arrList["total"] ; $i++ ) {
	...
	<tr>
		...
		<td style=mso-number-format:'\@'>".$arrInfo["list"][0]['shipping_date']."</td>
