...
<!-- 						<col class="w8p"> -->
					...
					<thead>
						<tr>							
							...
							<th class="pc_vw">단가</th>
							<th class="pc_vw">수량</th>
...
<!-- 							<td><i class="mo_vw">소비자가</i><?=number_format($arrList['list'][$i]['p_price'])?></td> -->
							...
							<td><input type="text" class="w1" style="width:80px;text-align:right;" id="qty" value="0"></td>			
							<td class="mono_btm"><i class="mo_vw">관리</i>
								<div class="btns">
									<a href="javascript:void(0);" onclick="fnInputCheck(this,'<?=$arrList['list'][$i]['idx']?>')" class="btn perf">선택</a>
								</div>
							</td>
						</tr>
					<?
...
<script type="text/javascript">
<!--
function fnInputCheck(ths,idx){
	var qty = $(ths).parent().parent().parent().children('td').eq(7).children('input');
	...
	if(qty.val() < 1){
		...
		//alert(idx+"/"+ qty.val())
		parent.fnGoodAdd(idx, qty.val());
		//parent.fnGoodSelect(idx,'P')
	}
}	
//-->
</script>
