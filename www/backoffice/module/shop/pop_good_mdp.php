...
function fnWritePage(){
	top.location.href = '/backoffice/module/shop/good_info.php';
}
//-->
</script>
<style type="text/css">
...
<div class="container">
	...
	<div class="title">전체 상품 목록 ( 상위 순서 3번 까지 노출됨 )</div><?##여기만 다름?>

	<div class="bdr_btm">
		<div class="btns">	
			<a href="javascript:void();" onclick="fnWritePage();" class="btn" style="margin: 10px;">상품등록</a>
				...
				<table>
					...
					?>
						<tr data-order=<?=$arrList['list'][$i]['idx']?>>
							...
							?></td>
							<td style="width:120px"><?=stripslashes($arrList['list'][$i]['g_code'])?></td>
							<td style="width:100px"><?=$simg?></td>
							<td style="text-align:left;width:470px"><strong><?=stripslashes($arrList['list'][$i]['g_name'])?></strong><br/><?=$arrOption[$i]?></td>							
					...
					?>
					<tr height="100">
						<td width="100%" colspan="6" style="width:1000px" >검색된 데이터가 없습니다.</td>
					</tr>
					<?}?>
					</tbody>
				</table>
