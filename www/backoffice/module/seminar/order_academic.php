				...
				<table>
					...
					<thead>
						<tr>							
							...
							<th class="pc_vw">수강기간</th>
							<th class="pc_vw">전체수강률</th>
							<th class="pc_vw">파일다운로드</th>		
							<th class="pc_vw">수료증발급</th>
					...
					if($arrList['list']['total'] > 0){
						for ($i=0;$i<$arrList['list']['total'];$i++){
							...
							$stateText = "";
							if($arrList['list'][$i]["g_opt_1"]=="free"){							
								$stateText = "제한없음";
							}else if($arrList['list'][$i]["g_opt_1"]=="30D"){
								$stateText = str_replace("-",".",substr($arrList['list'][$i]['order_date'],0,10))." ~ ".date("Y.m.d", strtotime($arrList['list'][$i]['order_date']." +30 days"));								
							}
							################################################## 세미나 정보 ##############################################
							################################################## 수강률 ##############################################
							$totSukang = 0;
							$studyInfo = studyList("def", $arrGoodInfo["list"][0]['rel_a_idx'], $arrList["list"][$i]['order_id']);
							for($j=0;$j<$studyInfo["total"];$j++){
								if($studyInfo['list'][$j]['curr']){
									$sukang = floor($studyInfo['list'][$j]['curr']/$studyInfo['list'][$j]['durat']*100);
								}else{
									$sukang = 0;
								}
							...
							if($studyInfo["total"]>0){
								$totSukang = floor($totSukang/$studyInfo["total"]);
							}
					...
					?>
						<tr>
							...
							<td><i class="mo_vw">수강기간</i><?=$stateText?></td>
							<td><i class="mo_vw">전체수강률</i><?=$totSukang?>%</td>
							<td><i class="mo_vw">파일다운로드</i><?=$arrList["list"][$i]['bdate']?"Y":""?></td>
							<td><i class="mo_vw">수료증발급</i><?=$arrList["list"][$i]['cdate']?"Y":""?></td>
							<td><i class="mo_vw">상태</i><?=$_SITE["SHOP"]["ORDER_STATE"][$arrList["list"][$i]['order_state']]?></td>							
							<td class="mono_btm"><i class="mo_vw">관리</i>
								<div class="btns">
									<a href="order_academic_detail.php?order_no=<?=$arrList['list'][$i]['order_no']?>&<?=$_SERVER['QUERY_STRING']?>" class="btn modi">수정</a>
