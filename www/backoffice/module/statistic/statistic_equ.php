...
?>
	...
	<div class="container">
		...
		<div class="bdr_top">
			...
			<div class="bdr_right">
				<div class="btns">
					<a href="/backoffice/module//board/board_statistic_equ_xls.php?year=<?=$year?>" class="excel" download>엑셀파일로 저장<span class="pc_vw"></span></a>
				</div>
			</div>
		</div>
							...
							if (!empty($monthData)): // 월 데이터가 있는 경우만 표시
									...
									if ($cat_no !== '소계'): // 소계는 나중에 처리
										foreach ($categoryData as $subject => $data):
											...
											<tr>
												...
												<!-- 대여일 -->
												<td><?= isset($data['days']['유료']) ? $data['days']['유료'] : 0 ?></td>
												<td><?= isset($data['days']['교육']) ? $data['days']['교육'] : 0 ?></td>
												<td><?= isset($data['days']['지원']) ? $data['days']['지원'] : 0 ?></td>
												<td><?= isset($data['days']['할인']) ? $data['days']['할인'] : 0 ?></td>
												<td><?= isset($data['days']['유료+지원']) ? $data['days']['유료+지원'] : 0 ?></td>
								...
								if (isset($monthData['소계'])): ?>
									<tr class="total">
										...
										<td><?= $monthData['소계']['days']['유료'] ?></td>
										<td><?= $monthData['소계']['days']['교육'] ?></td>
										<td><?= $monthData['소계']['days']['지원'] ?></td>
										<td><?= $monthData['소계']['days']['할인'] ?></td>
										<td><?= $monthData['소계']['days']['유료+지원'] ?></td>
