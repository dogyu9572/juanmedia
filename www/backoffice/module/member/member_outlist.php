				...
				<table>
					...
					?>
						<tr>
							...
							<td><i class="mo_vw">이름</i><?=$arrList['list'][$i]['user_name']?> <strong style="color:red;"><?=$newTxt?></strong></td>
							<td><i class="mo_vw">휴대폰번호</i><?=$arrList['list'][$i]['mobile']?></td>

							<td><i class="mo_vw">메일 수신동의</i><?=$arrList['list'][$i]['email_accept']?></td>
							<td><i class="mo_vw">SMS 수신동의</i><?=$arrList['list'][$i]['sms_accept']?></td>
							<td><i class="mo_vw">알림톡 수신동의</i><?=$arrList['list'][$i]['kakao_accept']?></td>
							<td><i class="mo_vw">탈퇴날짜</i><?=$arrList['list'][$i]['outdt']?><br><?//=$arrList['list'][$i]['user_editdt']?> </td>	
							<td class="mono_btm"><i class="mo_vw">관리</i>
								<div class="btns">
									<a href="member_info.php?user_id=<?=$arrList['list'][$i]['user_id']?>&listURL=<?=urlencode($_SERVER['REQUEST_URI'])?>" class="btn perf">확인</a>
